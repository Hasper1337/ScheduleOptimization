...
public static class ScheduleOptimizationService
{
    ...
    private static string BuildExplanation(IReadOnlyList<Lesson> lessons, List<DayPattern> patterns, int bestUsedDays)
    {
        ...
        sb.AppendLine("ψ[z,x,y] ∈ {0,1}: занятие z поставлено в день y на пару x.");
        ...
        sb.AppendLine("1) Каждое занятие ставится ровно один раз: Σ_y Σ_x ψ[z,x,y] = 1 для всех z∈Z.");
        sb.AppendLine("2) В одном слоте не более одного занятия: Σ_z ψ[z,x,y] ≤ 1 для всех x,y.");
        sb.AppendLine("3) Максимум 4 пары в день задаётся X={1..4}; минимум 0 допускается.");
        sb.AppendLine("4) Связь с учебным днём: Σ_z Σ_x ψ[z,x,y] ≤ 4·u[y] для всех y.");
        sb.AppendLine("5) Без окон: если пара x занята, то все предыдущие пары в этот день заняты.");
        sb.AppendLine("   В коде это реализовано шаблонами, где заняты только пары 1..k.");
        sb.AppendLine("6) Порядок в дне: лекции → практики → лабы; лекций в день не больше 2.");
        sb.AppendLine();
